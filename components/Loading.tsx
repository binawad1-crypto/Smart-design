...
const Loading: React.FC<LoadingProps> = ({ status, step, facts = [], uiLanguage = 'en' }) => {
  ...
  return (
    <div className="relative flex flex-col items-center justify-center w-full max-w-4xl mx-auto mt-8 min-h-[350px] md:min-h-[500px] overflow-hidden rounded-3xl bg-white/40 dark:bg-slate-900/40 border border-slate-200 dark:border-white/10 shadow-2xl backdrop-blur-md transition-colors" dir={uiLanguage === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="relative z-20 mb-10 md:mb-16 scale-[0.65] md:scale-125 mt-4 md:mt-10">
        ...
        <div className="absolute top-1/2 left-1/2 w-[300px] md:w-[500px] h-[300px] md:h-[500px] -translate-x-1/2 -translate-y-1/2 pointer-events-none">
           ...
           <FlyingItem content={t.introHistory} type="text" delay={0.2} position={2} />
           <FlyingItem content={Microscope} type="icon" delay={0.5} position={3} />
           <FlyingItem content={t.introScience} type="text" delay={0.7} position={4} />
           <FlyingItem content={Dna} type="icon" delay={1.0} position={5} />
           <FlyingItem content={t.introFacts} type="text" delay={1.2} position={6} />
           <FlyingItem content={Globe} type="icon" delay={1.5} position={7} />
           <FlyingItem content={t.introData} type="text" delay={1.7} position={8} />
      ...
      <div className="relative z-30 w-full max-w-lg bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-2xl p-6 md:p-8 shadow-2xl border border-slate-200 dark:border-white/10 text-center flex flex-col items-center transition-all duration-500 min-h-[140px] md:min-h-[160px]">
        ...
        <div className="w-full h-1 bg-slate-200 dark:bg-slate-800 mt-6 rounded-full overflow-hidden">
            <div 
              className={`h-full bg-gradient-to-r from-amber-400 via-cyan-500 to-emerald-500 transition-all duration-1000 ease-out relative overflow-hidden shadow-[0_0_10px_rgba(6,182,212,0.8)] ${uiLanguage === 'ar' ? 'float-right' : ''}`}
