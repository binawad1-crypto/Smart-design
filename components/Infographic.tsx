...
const Infographic: React.FC<InfographicProps> = ({ image, onEdit, isEditing, uiLanguage }) => {
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-6xl mx-auto animate-in fade-in zoom-in duration-700 mt-8">
      ...
      <div className="relative group w-full bg-slate-100 dark:bg-slate-900 rounded-2xl overflow-hidden shadow-2xl border border-slate-200 dark:border-slate-700/50">
        ...
        <div className={`absolute top-0 w-16 h-16 border-t-2 border-cyan-500/30 z-20 pointer-events-none ${uiLanguage === 'ar' ? 'right-0 border-r-2 rounded-tr-2xl' : 'left-0 border-l-2 rounded-tl-2xl'}`}></div>
        <div className={`absolute top-0 w-16 h-16 border-t-2 border-cyan-500/30 z-20 pointer-events-none ${uiLanguage === 'ar' ? 'left-0 border-l-2 rounded-tl-2xl' : 'right-0 border-r-2 rounded-tr-2xl'}`}></div>
        <div className={`absolute bottom-0 w-16 h-16 border-b-2 border-cyan-500/30 z-20 pointer-events-none ${uiLanguage === 'ar' ? 'right-0 border-r-2 rounded-br-2xl' : 'left-0 border-l-2 rounded-bl-2xl'}`}></div>
        <div className={`absolute bottom-0 w-16 h-16 border-b-2 border-cyan-500/30 z-20 pointer-events-none ${uiLanguage === 'ar' ? 'left-0 border-l-2 rounded-bl-2xl' : 'right-0 border-r-2 rounded-br-2xl'}`}></div>
        ...
        <div className={`absolute top-6 flex gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity z-30 ${uiLanguage === 'ar' ? 'left-6' : 'right-6'}`}>
          <button 
            onClick={() => setIsFullscreen(true)}
      ...
      <div className="w-full max-w-3xl -mt-6 sm:-mt-8 relative z-40 px-4">
        <div className="bg-white/90 dark:bg-slate-800/80 backdrop-blur-xl p-3 sm:p-2 sm:pr-3 rounded-2xl shadow-2xl border border-slate-200 dark:border-white/10 flex flex-col sm:flex-row gap-2 items-center ring-1 ring-black/5 dark:ring-white/5">
            <div className={`text-cyan-600 dark:text-cyan-400 hidden sm:block ${uiLanguage === 'ar' ? 'pr-4' : 'pl-4'}`}>
            ...
            <form onSubmit={handleSubmit} className="flex-1 w-full flex flex-col sm:flex-row gap-2">
                ...
                <div className="w-full sm:w-auto" title={!editPrompt.trim() ? t.refinePrompt : t.enhance}>
                    <button
                        type="submit"
