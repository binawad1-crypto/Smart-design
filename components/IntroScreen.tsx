...
const IntroScreen: React.FC<IntroScreenProps> = ({ onComplete, uiLanguage }) => {
  ...
  useEffect(() => {
    ...
    const timer4 = setTimeout(() => onComplete(), 7500); // Auto-enter after logo display
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-slate-950 flex flex-col items-center justify-center overflow-hidden font-display" dir={uiLanguage === 'ar' ? 'rtl' : 'ltr'}>
      <style>{`
      ...
      <div className={`absolute bottom-16 md:bottom-20 flex flex-col items-center transition-all duration-1000 px-6 ${phase === 3 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
         <h1 className="text-3xl md:text-5xl font-bold text-white mb-2 tracking-tight text-center">
            {t.introTitle} <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-amber-400">{t.introSubtitle}</span>
         </h1>
         
         <div className="flex flex-col items-center gap-3 mb-8 text-center">
            <p className="text-slate-400 text-xs md:text-sm uppercase tracking-[0.3em]">{t.introTagline}</p>
         </div>
      </div>
      ...
      <button 
        onClick={onComplete}
        className={`absolute top-6 text-[10px] md:text-xs text-slate-600 hover:text-white transition-colors uppercase tracking-widest border border-transparent hover:border-white/10 px-3 py-1 rounded-full ${uiLanguage === 'ar' ? 'left-6 md:left-8' : 'right-6 md:right-8'}`}
      >
        {t.skipIntro}
      </button>
